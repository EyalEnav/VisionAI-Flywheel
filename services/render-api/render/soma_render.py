...
def render(npz_path, out_video, texture_mode="cosmos", colors=None,
    ...
    # Default clothing palette if none provided
    if colors is None:
        colors = CROWD_PALETTES[0]
    ...
    # Always fixed mount; for traveling clips the camera is offset laterally so
    # the subject walks through the frame rather than being glued to center.
    # A lazy PTZ drift gives it the feel of a real pan-tilt unit.
    mid_xz = root_pos_all[T // 2]  # midpoint of journey
    ...
    _ptz_state = [float(mid_xz[0]), float(mid_xz[1])]
    ...
    for i in range(T):
        ...
        # ── Surveillance camera — fixed mount, lazy PTZ pan ──
