...
Waits for any active cosmos jobs to finish, then processes.
"""
import json, os, time, requests, subprocess

COSMOS_API = "http://localhost:8080"
RENDER_DIR = "/home/ubuntu/render_output"
OUT_DIR    = "/opt/dlami/nvme/cosmos_incidents"
STATE_FILE = "/opt/dlami/nvme/cosmos_incidents/batch_state.json"
...
def main():
    ...
    for job_uuid, clip_name in CLIPS.items():
        ...
        for bg_id, bg_prompt in BACKGROUNDS:
            ...
            if os.path.exists(output_host) and os.path.getsize(output_host) > 10000:
                print(f"  file exists: {key}", flush=True)
                state[key] = {"status": "done", "output": output_host}
                save_state(state)
                done_count += 1
                continue
